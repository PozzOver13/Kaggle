...
embedding_paragram_filtered = \
    read_and_filter_embedding(EMBEDDING_FILE_PARAGRAM,
                              train_vocabulary_paragram,
                              source = "paragram")

# check % of words found in embedding
check_coverage(train_vocabulary_paragram, embedding_paragram_filtered)

# > NN TRAINING (PARAGRAM)
...
train_kit_paragram = fit_tokenizer(train_corpus_cl, 
                                embedding_paragram_filtered,
                                max_features=max_features,
                                maxlen=sentences_maxlen,
                                add_filters="'’”")

train_X = train_kit_paragram['train_matrix']

val_X = apply_tokenizer(train_kit_paragram['tokenizer'], 
                        val_corpus, sentences_maxlen)

test_X = apply_tokenizer(train_kit_paragram['tokenizer'], 
...
x = Embedding(max_features, EMBED_SIZE, 
              weights=[train_kit_paragram['embedding_matrix']])(inp)
x = Bidirectional(CuDNNGRU(64, return_sequences=True, stateful=True))(x)
x = GlobalMaxPool1D()(x)
x = Dense(16, activation="relu")(x)
x = Dropout(0.1)(x)
x = Dense(1, activation="sigmoid")(x)
model_nn_paragram = Model(inputs=inp, outputs=x)
model_nn_paragram.compile(loss='binary_crossentropy', 
                       optimizer='adam', metrics=['accuracy'])
print(model_nn_paragram.summary())
...
model_nn_paragram.fit(train_X[0:1110016,:], train_y[0:1110016],
